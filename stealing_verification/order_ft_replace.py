...
def main():
    ...
    if args.featured_proportion > 0:
        ...
        train_dataset = poisoned_datasets['replace_train_dataset']
    ...
    # print('len poisoned_eval_dataset', len(poisoned_eval_dataset))
    ...
    model, optimizer, train_dataloader, eval_dataloader, poisoned_eval_dataloader, lr_scheduler = accelerator.prepare(
        model, optimizer, train_dataloader, eval_dataloader, poisoned_eval_dataloader, lr_scheduler
    )
    ...
    logger.info(f"  Target label = {args.target_label}")
    logger.info(
    ...
    def asr_epoch():
        model.eval()
        for step, batch in enumerate(poisoned_eval_dataloader):
            with torch.no_grad():
                with autocast(device_type='cuda'):
                    try:
                        outputs = model(**batch).logits
    ...
    def evaluator(model):
        ...
        if args.featured_proportion != 0:
            asr = asr_epoch()
            logger.info(
                f"Epoch {epoch} step {completed_steps}: eval_acc: {eval_acc:.4f} asr: {asr:.4f} task_loss: {task_loss:.4f}")
            accelerator.log(
                {
                    "eval_acc": eval_acc,
                    "asr": asr,

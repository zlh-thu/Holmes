...
from offsite_tuning.models.resnet import ResNet
import gc
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        dataset = {
            ...
            "test": pt_datasets.ImageFolder(root=args.validation_dir, transform=val_transforms),
        }
    else:
        data_files = {}
        ...
        if args.validation_dir is not None:
            data_files["test"] = os.path.join(args.validation_dir, "**")
    ...
    # If we don't have a validation split, split off a percentage of train as test.
    args.train_val_split = None if "test" in dataset.keys() else args.train_val_split
    ...
    if 'resnet' in args.model_name_or_path:
        # raw_model = ResNetForImageClassification.from_pretrained(args.model_name_or_path)
        ...
        raw_model = ResNetForImageClassification(config)
        model = ResNet(config, raw_model)
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.num_train_epochs)
    ...
    def eval_epoch():
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                #print('batch["pixel_values"].device', batch["pixel_values"].device)
                #print('model.device', model.device)
                with autocast():
                    outputs = model(**batch)
                # outputs = model(batch["pixel_values"].half())
            # print(outputs)
    ...
    if accelerator.is_main_process:
        for param_tuple in model.named_parameters():
            ...
            print("name = ", name, ' shape', param.shape, 'grad', param.requires_grad)

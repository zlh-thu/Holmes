...
def add(self, other: Iterable[nn.Module]) -> 'nn.ModuleList':
    combined = nn.ModuleList()
...
def setup_teacher_student(model, args, accelerator, load_benign=False):
    for param in model.parameters():
    ...
    if hasattr(args, 'benign_load_student') or hasattr(args, 'sus_load_student'):
        if load_benign:
            load_student_dir = args.benign_load_student
        else:
            load_student_dir = args.sus_load_student
    else:
        load_student_dir = args.load_student

    if load_student_dir and 'res' not in args.model_name_or_path:
        student_state_dict = torch.load(os.path.join(
            load_student_dir, 'student.pt'), map_location='cpu')
        student_layers_len = len(
            set([k.split('.')[0] for k in student_state_dict.keys()]))
        logger.info(
            f"Loading student module from {load_student_dir} with {student_layers_len} layers.")
    ...
    model.adapter = add(layers[:l], layers[r:])
    # model.adapter = ModuleList(layers[:l]) + ModuleList(layers[r:])
    ...
    if args.train_module == 'student':
        ...
        model.trainable_module = add(model.student, model.adapter)
    else:
        raise NotImplementedError
...
def to_teacher(model, args):
    ...
    if isinstance(model, OPTForCausalLM):
        ...
        # model.module.vit.encoder.layers = model.module.vit.encoder.layers[:l] + \
        #    model.module.teacher + model.module.vit.encoder.layers[r:]
        model.vit.encoder.layers = add(model.vit.encoder.layers[:l], add(model.teacher, model.vit.encoder.layers[r:]))
    elif isinstance(model, EVAViTForImageClassification):
        r = len(model.blocks) - args.student_r_pad
        model.blocks = model.blocks[:l] + \
                       model.teacher + model.blocks[r:]
    elif isinstance(model, DistributedDataParallel) and isinstance(model.module, CLIPViTForImageClassification):
        r = len(model.module.vit.encoder.layers) - args.student_r_pad
        # model.module.vit.encoder.layers = model.module.vit.encoder.layers[:l] + \
        #    model.module.teacher + model.module.vit.encoder.layers[r:]
        model.module.vit.encoder.layers = add(model.module.vit.encoder.layers[:l],
                                              add(model.module.teacher, model.module.vit.encoder.layers[r:]))
...
def to_student(model, args):
    ...
    if isinstance(model, OPTForCausalLM):
        ...
        r = len(model.vit.encoder.layers) - args.student_r_pad
        model.vit.encoder.layers = add(model.vit.encoder.layers[:l],
                                       add(model.student, model.vit.encoder.layers[r:]))
    elif isinstance(model, EVAViTForImageClassification):
        r = len(model.blocks) - args.student_r_pad
        model.blocks = model.blocks[:l] + \
        ...
        r = len(model.module.vit.encoder.layers) - args.student_r_pad
        model.module.vit.encoder.layers = add(model.module.vit.encoder.layers[:l],
                                              add(model.module.student, model.module.vit.encoder.layers[r:]))
...
def get_kd_loss(model):
    if isinstance(model, DistributedDataParallel):
        kwargs = model.module.student_l.input_kwargs
        args = model.module.student_l.input_args
    else:
    ...
    with torch.no_grad():
        if isinstance(model, DistributedDataParallel):
            ...
            for teacher_layer in model.module.teacher:
    ...
    if isinstance(model, DistributedDataParallel):
        output_student = model.module.student_r.cached_output.float()
    else:
...
def setup_trainable_layers(model, args, accelerator):
    for param in model.parameters():
        param.requires_grad = False

    layers = get_layers(model)

    l, r = args.trainable_l_pad, len(layers) - args.trainable_r_pad
    if args.load_model:
        state_dict = torch.load(os.path.join(
            args.load_model), map_location='cpu')
        logger.info(
            f"Loading model from {args.load_model}.")
...
def setup_teacher_student_by_dir(model,
                                 student_l_pad,
                                 student_r_pad,
                                 accelerator,
                                 load_student,
    ...
    l, r = student_l_pad, len(layers) - student_r_pad

    ### with ownership verification ###

    load_student_dir = load_student

    if load_student_dir and 'res' not in model_name_or_path:
        student_state_dict = torch.load(os.path.join(
            load_student_dir, 'student.pt'), map_location='cpu')
        student_layers_len = len(
            set([k.split('.')[0] for k in student_state_dict.keys()]))
    ...
    if student_layer_selection_strategy == 'uniform':
        student = uniform_choose_layers(student, num_student_layers)
    else:
        raise NotImplementedError

    student = student.to(accelerator.device)

    if magnitude_pruning_ratio > 0:
        logger.info(
            f"Pruning student module with magnitude ratio {magnitude_pruning_ratio}")
        magnitude_prune(student, magnitude_pruning_ratio)

    if weight_quantization_bits is not None:
        logger.info(
            f"Quantizing student module with {weight_quantization_bits} bits")
        quantize(student, weight_quantization_bits)

    if train_module == 'student':
        for param in student.parameters():
            param.data = param.data.float()
            param.requires_grad = True
    elif train_module == 'adapter':
        for param in student.parameters():
            param.requires_grad = False
...
def to_student_dir(model, student_l_pad, student_r_pad):
    l = student_l_pad
    if isinstance(model, OPTForCausalLM):
        ...
        r = len(model.module.vit.encoder.layers) - student_r_pad
        model.module.vit.encoder.layers = add(model.module.vit.encoder.layers[:l],
                                              add(model.module.student, model.module.vit.encoder.layers[r:]))
